...
extension APIService {
    ...
    func request(showIndicator:Bool = true, isJsonRequest: Bool = false, success: @escaping APIServiceSuccessCallback, failure: @escaping APIServiceFailureCallback) {
        if isJsonRequest {
            self.jsonRequest(showIndicator: showIndicator,jsonSuccess: { (responseSuccess) in
            ...
            self.requestNormal(showIndicator: showIndicator, normalSuccess: { (responseSuccess) in
                success(responseSuccess)
    ...
    func request(mediaDict:[String: Data]? = nil, mediaArray:[Data]? = nil, mediaName: String? = nil, success:  @escaping APIServiceSuccessCallback, failure: @escaping APIServiceFailureCallback) {
        ...
        AF.upload((urlRequest?.1)!, with: (urlRequest?.0)!).uploadProgress(closure: { (progress) in
            ...
            let _ = ["value": progress.fractionCompleted]
    ...
    //MARK: JSON REUQEST
    private func jsonRequest(showIndicator:Bool = true,jsonSuccess: @escaping APIServiceSuccessCallback, jsonFailure: @escaping APIServiceFailureCallback) {
        do {
            if showIndicator {
                self.showIndicator() //Show Indicator
            }
    ...
    //MARK: NORMAL REUQEST
    private func requestNormal(showIndicator:Bool = true,normalSuccess: @escaping APIServiceSuccessCallback, normalFailure: @escaping APIServiceFailureCallback) {
        //Show Indicator
        if showIndicator {
            self.showIndicator()
        }
