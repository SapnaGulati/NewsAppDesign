//
//  APIKeys.swift
//  NewsApp
//
//  Created by ios4 on 15/06/21.
//

import Foundation

class APIKeys {
    static let kMessage = "message"
    static let kUserId = "userId"
    static let kAuthorizationToken = "Authorization"
    static let kAccessToken = "537f81739ffa4a09b7bc1511841e6729"
//    static let kAccessToken = "4d3e1ce2523f46418ff4a356b80f556d"
    static let kStatus = "status"
    static let kFacebookLogIn = "facebooklogin"
    static let kGoogleLogIn = "googlelogin"
    static let kAppleLogIn = "applelogin"
    static let kCategory = "category"
    static let kCategoryIndex = "categoryindex"
    static let kCountry = "country"
    static let kCountryIndex = "countryindex"
    static let kData = "data"
    static let kId = "id"
    static let kCode = "code"
    static let kName = "name"
    static let kFlag = "flag"
    static let kTotalResults = "totalresults"
    static let kArticles = "articles"
    static let kAuthor = "author"
    static let kTitle = "title"
    static let kDescription = "description"
    static let kPublishedAt = "publishedAt"
    static let kURLToImage = "urlToImage"
    static let kURL = "url"
    static let kSource = "source"
    static let kSourceName = "name"
    static let kLanguage = "language"
    static let kSources = "sources"
}
